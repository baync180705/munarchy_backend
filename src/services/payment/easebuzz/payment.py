...
def _payment(params, merchant_key, salt, env):
    ...
    # empty validation
    empty_validation = _emptyValidation(params, salt)

    if empty_validation != True:
        return empty_validation

    # check amount should be in floating formate
    if re.match(r"^([\d]+)\.([\d]?[\d])$", params['amount']):
        params['amount'] = float(params['amount'])

    # type validation
    type_validation = _typeValidation(params, salt, env)
    ...
    email_validation = _email_validation(params['email'])
    ...
    pay_result = _pay(params, salt, URL)
